...
WORKDIR /src
COPY ["deploymentapp.csproj", "."]
RUN dotnet restore "./deploymentapp.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./deploymentapp.csproj" -c %BUILD_CONFIGURATION% -o /app/build

# Cette étape permet de publier le projet de service à copier dans la phase finale
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./deploymentapp.csproj" -c %BUILD_CONFIGURATION% -o /app/publish /p:UseAppHost=false
